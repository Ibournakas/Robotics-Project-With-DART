...
def configure(conf):
    ...
    avx_robot_dart = conf.check_avx(lib='robot_dart', required=['RobotDARTSimu', 'RobotDARTMagnum'], lib_type='static')
    ...
    if avx_dart and avx_robot_dart:
        ...
        conf.msg('-march=native (AVX support)', 'yes, but either DART or robot_dart are not compiled with native flags', color='YELLOW')
        native = ' -march=native'
        native_icc = ' mtune=native'
    else:
...
def build(bld):
    if len(bld.env.INCLUDES_DART) == 0 or len(bld.env.INCLUDES_EIGEN) == 0 or len(bld.env.INCLUDES_BOOST) == 0 or len(bld.env.INCLUDES_ROBOT_DART) == 0:
    ...
    bld.program(features = 'cxx',
                install_path = None,
                source = '/src/cpp/cube_stacking.cpp',
                includes = './src',
                uselib = libs,
                defines = [],
                target = 'cube_stacking')
